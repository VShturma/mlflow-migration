...
class HttpClient(BaseHttpClient):
    ...
    def _delete(self, resource):
        uri = self._mk_uri(resource)
        rsp = requests.delete(
            uri, headers=self._mk_headers(), auth=self.auth, timeout=_TIMEOUT
        )
    ...
    def _mutator(self, method, resource, data=None):
        uri = self._mk_uri(resource)
        rsp = method(
            uri, headers=self._mk_headers(), data=data, auth=self.auth, timeout=_TIMEOUT
        )
        return self._check_response(rsp)
    ...
    def _check_response(self, rsp, params=None):
        if rsp.status_code < 200 or rsp.status_code > 299:
            ...
            raise MlflowExportImportException(
                json.dumps(msg), http_status_code=rsp.status_code
            )
        return rsp
...
@click.option(
    "--method", help="HTTP method: GET|POST|PUT|PATCH|DELETE.", type=str, default="GET"
)
@click.option(
    "--params", help="HTTP GET query parameters as JSON.", type=str, required=False
)
@click.option(
    "--data",
...
@click.option("--output-file", help="Output file.", type=str, required=False)
